      ...
      for await (const { key, update } of args) {
        ...
        if (key.remoteJid !== 'status@broadcast' && key.id !== undefined) {
          ...
          if (configDatabaseData.HISTORIC || configDatabaseData.NEW_MESSAGE) {
            ...
            if (!findMessage?.id) {
              this.logger.verbose(
                `Original message not found for update after 3 retries. Skipping. This is expected for protocol messages or ephemeral events not saved to the database. Key: ${JSON.stringify(key)}`,
              );
              continue;
            }
